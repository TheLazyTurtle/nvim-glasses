...
local create_razorpage_for_all_templates = function(project, model, name, template)
    ...
    vim.ui.select(db_context_list.find_db_context(project), opts, function(db_context)
        ...
        if name == nil then
            local cmd = string.format("~/.dotnet/tools/dotnet-aspnet-codegenerator razorpage -p %s -m %s -dc %s -outDir Views/%ss", project, model_name, db_context, model_name)
            local results = vim.fn.systemlist(cmd)
            print(vim.inspect(results))
        else
            local cmd = string.format("~/.dotnet/tools/dotnet-aspnet-codegenerator razorpage -p %s %s %s -m %s -dc %s -outDir Views/%ss", project, name, template, model_name, db_context, model_name)
            local results = vim.fn.systemlist(cmd)
            print(vim.inspect(results))
        end
    end)
...
local get_page_name = function(project, model, template)
    ...
    vim.ui.input(opts, function(name)
        create_razorpage_for_all_templates(project, model, name, template)
    end)
end

local get_template_type = function(project, model)
    local templates = { "Create", "Edit", "Delete", "Details", "List" }
...
local one_or_all = function(project, model)
    ...
    vim.ui.select(types, opts, function(template)
        if template == "Specific template" then
            ...
            create_razorpage_for_all_templates(project, model)
        end
    end)
end
